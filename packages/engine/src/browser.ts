 ...
 *
 * The name must hold the SAME object that the engine reads. A copy splits the two: the worker
 * writes DL.maxCells when the page says how much memory it can use, and the operations read
 * DL.maxCells. With two objects the operations keep the value that the engine started with.
 */
...
if (before) {
  ...
  // not, the browser holds an engine from another version, which gives old answers with no
