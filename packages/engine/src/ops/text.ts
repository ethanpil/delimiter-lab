...
/* ---------- Replace ---------- */
// The pairs that find something: find and replace first, then the rows of "more". A saved step
// from before the list has no list, and then only the first pair runs.
function replacePairs(p) {
  return [{ from: p.find, to: p.replace }].concat(p.more || []).filter(function (m) { return !!m.from; });
}
...
DL.registerOp({
  ...
  validate: function (p) {
    ...
    if (!p.regex) return [];
    // The rows have the numbers that the form shows: find and replace are row 1, and the form does not
    // show an empty row of the list.
    var problems = [];
    var n = 0;
  ...
  apply: function (table, p) {
    ...
    var steps = replacePairs(p).map(function (m) { return replacer(m.from, m.to, p); });
    var stats: any = {};
...
DL.registerOp({
  ...
  params: [
    ...
    { key: 'steps', label: 'Clean', type: 'checkboxes', fill: true, default: ['html', 'control', 'spaces', 'unicode'],
