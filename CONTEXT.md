...
- **Persistence.** `localStorage` keys: `dl.session.v1` (steps, selection, source options and file name, not the file), `dl.workflows.v1` (the saved records: id, name, steps, columns, sourceOptions, createdAt, updatedAt, lastUsedAt, uses), `dl.theme` and `dl.autosave`. Every reader of these keys filters bad records; a shape change needs a migration there. On restore, a workflow with an id is compared with its record to decide the saved state; steps of unknown operations are dropped with a toast.
- **Autosave.** The header button writes the open workflow to its record after every change to the steps or the settings, 800 ms after the last change. It acts only when `workflow.id` exists. Turning it on for a workflow that has no record opens the save dialog first; the button stays off when the user stops there. `saveWorkflow(then)` gives the record to `then`, or null when nothing was saved.
