...
- **New workflow.** The New button in the header empties the steps after two questions. It writes a change that waits with `autosaveNow()` first, so the old record keeps the work. The file that is open stays open.
