...
(function (root) {
  ...
  function GridView(container, engine, opts) {
    ...
    this.reset();
    var self = this;
    // Rendering ~40 rows is cheap, so render on every scroll event. (requestAnimationFrame
    // pauses in hidden tabs and would leave the grid stale.)
  ...
  GridView.prototype.fetchPage = function (pageIndex) {
    ...
    var p = this.engine.slice(stepId, pageIndex * PAGE, PAGE).then(function (msg) {
      self.inflight.delete(pageIndex);
  ...
  GridView.prototype.renderRows = function () {
    if (!this.stepId || !this.total) { U.empty(this.rowsEl); return; }
    ...
    var missing = [];
    for (var p = firstPage; p <= lastPage; p++) if (!this.pages.has(p)) missing.push(p);
    if (missing.length) {
      var version = this.showVersion;
    ...
    for (var i = first; i < last; i++) {
      ...
      if (row) {
        for (var c = 0; c < w; c++) {
          ...
          html += '<div class="' + cls + '" style="width:' + this.widths[c] + 'px" title="' + U.esc(v) + '">' + U.esc(v) + '</div>';
        }
      } else {
