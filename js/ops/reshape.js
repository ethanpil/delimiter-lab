...
(function (root) {
  ...
  // One cell of the pivot result: numbers, texts and a count.
  function Cell() {
    this.sum = 0;
    this.n = 0;
    this.min = Infinity;
  ...
  DL.registerOp({
    ...
    params: [
      { key: 'rows', label: 'Group rows by', type: 'columns', help: 'The result has one row for each different combination of these values.' },
      ...
      { key: 'value', label: 'Values to add up', type: 'column', required: false, help: 'Leave it empty to count rows.' },
    ...
    summary: function (p) {
      return p.aggregate + (p.value ? ' of ' + p.value : '') + ' by ' + p.rows.join(', ') + (p.columnKey ? ' × ' + p.columnKey : '');
    },
    validate: function (p) {
      var problems = [];
      if (!p.value && p.aggregate !== 'count') problems.push('Choose a column for "Values to add up", or set "Calculate" to Count.');
    ...
    apply: function (table, p) {
      ...
      for (i = 0; i < n; i++) {
        ...
        addValue(cell, valueGetter(i), agg);
      }

      var columns = p.rows.slice();
  ...
  DL.registerOp({
    ...
    apply: function (table, p) {
      ...
      var columns = DL.getOp('unpivot').outputColumns(table.columns, p);
