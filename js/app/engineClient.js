...
(function (root) {
  ...
  EngineClient.prototype.run = function (steps, protect, runId) { return this.send({ type: 'run', steps: steps, protect: protect, runId: runId }); };
  // Asks the worker to stop a run before its next step. The run replies with the results so far.
  EngineClient.prototype.cancel = function (runId) { return this.send({ type: 'cancel', runId: runId }); };
