...
(() => {
  ...
  function codeProblems(code) {
    ...
    try {
      new Function("row", "index", "num", "date", code);
    } catch (e) {
  ...
  function jsCell(v) {
    return v != null && typeof v === "object" && !(v instanceof Date) ? JSON.stringify(v) : DL.cellText(v);
  }
  DL.registerOp({
    id: "javascript",
  ...
  DL.registerOp({
    ...
    summary: function() {
      return "Edit rows with code";
    },
    validate: function(p) {
      return codeProblems(p.code);
    },
    outputColumns: function(cols) {
      return cols;
    },
    apply: function(table, p) {
      var fn = new Function("row", "index", "num", "date", p.code);
      ...
      var place = /* @__PURE__ */ Object.create(null);
      for (var c = 0; c < w; c++) place[cols[c]] = c;
      var out = new Array(w);
      var keep = null;
      var cells = 0, rows = 0, errors = 0, firstError = "";
      var unknown = /* @__PURE__ */ Object.create(null);
      for (var i = 0; i < n; i++) {
        var row = new Row();
        ...
        for (var key in row) {
          ...
          if (!out[at]) {
            out[at] = new Array(n);
            for (var j = 0; j < n; j++) out[at][j] = get[at](j);
          }
          out[at][i] = value;
          cells++;
          touched = true;
      ...
      var t = DL.makeTable(cols, table.cols.map(function(col, ci) {
        return out[ci] || col;
      }), n);
      ...
      if (names.length) {
        notes.push("No column has the name " + names.slice(0, 5).map(function(s) {
          return '"' + s + '"';
        }).join(", ") + (names.length > 5 ? " and " + (names.length - 5) + " more" : "") + ", so the code wrote nothing there. To add a column, use Custom JavaScript Column.");
      }
      return { table: t, notes, status: errors || names.length ? "warning" : "ok" };
    }
  });
  function Row() {
  }
